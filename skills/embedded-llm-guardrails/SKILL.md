...
如果项目使用 Claude Code，可额外检查 `.claude/settings.json` 是否存在。它仅用于 Claude Code 权限配置，不作为 Cursor 或 Codex 的公共安装目录。
...
4. 在当前项目使用的 IDE/Agent 入口文件中添加引用（仅维护 Claude Code、Cursor、Codex）。
...
并在 IDE/Agent 入口文件中引用它们。当前仅维护 Claude Code、Cursor、Codex 的入口方式，参考 `references/ide-entrypoints.md`。

Claude Code 的 skill 本体仍安装到 `.claude/skills/`，Cursor 规则入口放到 `.cursor/rules/`，Codex 入口放到 `AGENTS.md`。这些入口文件只引用 `.ai/` 下的同一套规则和边界文件。
