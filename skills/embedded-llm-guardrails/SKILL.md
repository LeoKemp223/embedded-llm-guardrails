...
读取项目 `.claude/` 目录是否存在以下文件：

- `.claude/LLM_RULES.md`
- `.claude/LLM_BOUNDARY.md`
- `.claude/settings.json`

> **注意：** 如果项目根目录存在旧版的 `LLM_RULES.md` 和 `LLM_BOUNDARY.md`，提示用户迁移到 `.claude/` 目录下。
...
**如果项目没有 `.claude/LLM_RULES.md` 和 `.claude/LLM_BOUNDARY.md`：**
...
#### settings.json 权限配置生成

根据项目类型和工具链自动生成 `.claude/settings.json`：
...
1. 读取项目规则：优先查看项目 `.claude/` 目录下的 `LLM_RULES.md` 和 `LLM_BOUNDARY.md`。
2. 如果没有项目边界配置，停止修改代码，要求用户先运行初始化流程。
3. 判断需求是否复杂或模糊；如果是，先梳理需求、拆分最小目标、列出待确认问题，等用户确认。
4. 明确本次 `Allowed Files` 和 `Forbidden Files / Areas`；未显式允许的文件默认禁止修改。以文本格式向用户复述本次边界。
5. 识别已验证代码标记，例如 `VERIFIED_ON_HARDWARE`、`DO_NOT_TOUCH_TIMING`、`PRODUCTION_VERIFIED`。
6. 如果涉及外设驱动，先梳理开发流程、已知信息和缺失信息，等用户确认后再生成代码。确认硬件参数时，必须覆盖从 MCU 引脚到目标器件的完整信号路径，不能只确认终端器件就假设中间环节。未确认的环节视为未知参数，按「不假设」规则处理。
7. 如果涉及 `main` 入口或主 `while` 循环，只允许添加带注释的函数调用；复杂逻辑必须放到独立函数。
8. 如果项目使用代码生成器（如 CubeMX），确认修改在 `USER CODE` 区域内；如果需要修改生成区域，提醒用户通过代码生成器操作。
9. 按 TDD 思路先定义最小验证方式，再实现刚好能通过验证的最小代码。
10. 修改后说明变更文件、验证方式、硬件风险和下一步扩展前需要用户确认的内容。
